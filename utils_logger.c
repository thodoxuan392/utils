...
void utils_log_log(utils_log_level_t _level, const char *file, int line, const char *fmt, ...){
    ...
    int size = snprintf(log_buffer , UTILS_MAX_LOG_BUFFER, "%s%d [%s] %s:%d: %s%s" ,level_to_color(_level), UTILS_LOG_TIMESTAMP() , level_to_str(_level) , file, line, message_buffer,level_to_color(UTILS_LOG_OFF));
